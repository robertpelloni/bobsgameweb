...
export class GameLogic extends EventEmitter<GameLogicEvents> {
    ...
    /** Whether the last successful move was a rotation (for T-spin detection) */
    public lastMoveWasRotation: boolean = false;
    /** Detected T-spin type: 0=none, 1=mini, 2=full */
    public lastTSpin: number = 0;
    /** Back-to-back difficult clear counter */
    ...
    private removeFlashedChainBlocks(): void {
        ...
        // T-spin announcements
        if (this.lastTSpin >= 1 && linesCleared > 0) {
            const tspinLabel = this.lastTSpin >= 2 ? 'T-Spin' : 'T-Spin Mini';
            if (linesCleared === 1) this.emit('announcement', `${tspinLabel} Single!`);
            if (linesCleared === 2) this.emit('announcement', `${tspinLabel} Double!`, BobColor.yellow);
            if (linesCleared === 3) this.emit('announcement', `${tspinLabel} Triple!`, BobColor.orange);
        } else {
        ...
        // Back-to-back bonus (consecutive difficult clears: 4-line or T-spin)
        if (linesCleared >= 4 || (this.lastTSpin >= 2 && linesCleared >= 2)) {
            this.backToBackCount = (this.backToBackCount ?? 0) + 1;
            if (this.backToBackCount >= 2) {
                this.emit('announcement', `B2B x${this.backToBackCount}!`, BobColor.magenta);
            }
    ...
    private movePiece(move: MovementType): boolean {
        ...
        // Reset last-move-rotation flag at start of each move
        const isRotation = move === MovementType.ROTATE_CLOCKWISE || move === MovementType.ROTATE_COUNTERCLOCKWISE;
        ...
        if (this.grid.doesPieceFit(this.currentPiece)) {
            this.lastMoveWasRotation = isRotation;
            this.emit('pieceMoved', this.currentPiece, move);
            return true;
        } else {
    ...
    /**
     * Detect T-spin using the 3-corner rule (SRS guideline).
     * A T-spin is when the last move was a rotation and at least 3 of the 4
     * corners diagonally adjacent to the T-piece center are occupied.
     * Returns 0 (none), 1 (mini), or 2 (full T-spin).
     */
    private detectTSpin(): number {
        if (!this.currentPiece || !this.lastMoveWasRotation) return 0;
        if (this.currentPiece.pieceType.name !== 'T') return 0;

        const cx = this.currentPiece.xGrid;
        const cy = this.currentPiece.yGrid;

        // Check 4 diagonal corners relative to T center
        const corners = [
            [cx - 1, cy - 1],
            [cx + 1, cy - 1],
            [cx - 1, cy + 1],
        ...
        for (const [ox, oy] of corners) {
            if (ox < 0 || ox >= this.grid.getWidth() || oy < 0 || oy >= this.grid.getHeight()) {
                occupied++; // Wall counts as occupied
            } else if (this.grid.get(ox, oy) !== null) {
                occupied++;
            }
        }

        if (occupied >= 3) {
            // Full T-spin: the two corners in the "forward" direction of the T are both filled
            const rot = this.currentPiece.currentRotation;
            // Forward corners based on rotation: 0=up, 1=right, 2=down, 3=left
            const fwd = rot === 0 ? [corners[0], corners[1]] :
                        rot === 1 ? [corners[1], corners[3]] :
                        rot === 2 ? [corners[2], corners[3]] :
                                    [corners[0], corners[2]];
            const fwdOccupied = fwd.filter(([ox, oy]) => {
                if (ox < 0 || ox >= this.grid.getWidth() || oy < 0 || oy >= this.grid.getHeight()) return true;
                return this.grid.get(ox, oy) !== null;
            }).length;

            return fwdOccupied >= 2 ? 2 : 1; // full vs mini
        }
        return 0;
    }
    ...
    private setPiece(): void {
        ...
        this.lastTSpin = this.detectTSpin();
