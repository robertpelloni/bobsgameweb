...
export class BattleScene extends Scene<BattleSceneConfig> {
    ...
    private createUI(): void {
        ...
        // Action Menu (4 options with cursor)
        this.actionMenu = new Container();
        const menuBg = new Graphics();
        menuBg.rect(0, 0, 240, 180);
        menuBg.fill({ color: 0x000000, alpha: 0.85 });
        menuBg.stroke({ color: 0x00ff00, width: 2 });
        this.actionMenu.addChild(menuBg);
        
        this.menuOptions = ['ATTACK', 'MAGIC', 'ITEMS', 'FLEE'];
        this.menuCursorPos = 0;
        this.menuTexts = [];
        for (let i = 0; i < this.menuOptions.length; i++) {
            const t = new Text({ text: (i === 0 ? '▶ ' : '  ') + this.menuOptions[i], style: { fill: '#00ff00', fontSize: 20, lineHeight: 36 } });
            t.position.set(20, 15 + i * 40);
            this.actionMenu.addChild(t);
            this.menuTexts.push(t);
        }
        
        this.actionMenu.position.set(40, this.height - 220);
    ...
    protected onUpdate(dt: number): void {
        ...
        if (this.turn === 'player') {
            ...
            if (InputManager.isUpPressed()) {
                this.menuCursorPos = (this.menuCursorPos - 1 + this.menuOptions.length) % this.menuOptions.length;
                this.updateMenuCursor();
            }
            if (InputManager.isDownPressed()) {
                this.menuCursorPos = (this.menuCursorPos + 1) % this.menuOptions.length;
                this.updateMenuCursor();
            }

            if (InputManager.isActionPressed() || InputManager.isKeyPressed(Key.Enter)) {
                this.executeMenuAction(this.menuCursorPos);
            }
    ...
    private updateMenuCursor(): void {
        for (let i = 0; i < this.menuTexts.length; i++) {
            this.menuTexts[i].text = (i === this.menuCursorPos ? '▶ ' : '  ') + this.menuOptions[i];
        }
    }
    ...
    private executeMenuAction(index: number): void {
        switch (index) {
            ...
            case 1: this.playerMagic(); break;
            case 2: this.playerUseItem(); break;
    ...
    private playerMagic(): void {
        this.turn = 'animating';

        // Determine available spells based on player level
        const level = this.config.player.level ?? 1;
        const spells = [
            { name: 'Fire', dmg: 15 + level * 3, cost: 5, color: 0xff4400 },
            { name: 'Ice', dmg: 12 + level * 2, cost: 4, color: 0x44aaff },
            { name: 'Lightning', dmg: 20 + level * 4, cost: 8, color: 0xffff00 },
        ].filter((_, i) => i < level); // Unlock spells with levels

        // Auto-pick strongest available spell
        const spell = spells[spells.length - 1];
        if (!spell) {
            this.logMessage('No spells available!');
            this.turn = 'player';
            return;
        }

        const mp = (this.config.player as any).mp ?? 99;
        if (mp < spell.cost) {
        ...
        if ((this.config.player as any).mp !== undefined) {
            (this.config.player as any).mp -= spell.cost;
        }

        this.logMessage(`You cast ${spell.name}!`);
        this.animateLunge(this.playerSprite, this.pOriginalX, this.pOriginalY, 1, () => {
            const dmg = spell.dmg + Math.floor(Math.random() * 5);
            this.config.enemy.hp -= dmg;

            this.triggerShake(12);
            InputManager.vibrate(0, 200, 0.6, 0.9);

            this.spawnDamageNumber(this.eOriginalX, this.eOriginalY - 50, dmg, spell.color);
            if (AudioManager.isLoaded('puzzle_tetris')) AudioManager.playSound('puzzle_tetris', { volume: 0.3 });

            this.enemySprite.tint = spell.color;
            setTimeout(() => this.enemySprite.tint = 0xffffff, 400);

            this.updateHealthBars();
            this.logMessage(`${spell.name} deals ${dmg} damage!`);
    ...
    private playerUseItem(): void {
        ...
        const hp = this.config.player.hp;
        const maxHp = this.config.player.maxHp;
        const healAmount = Math.floor(maxHp * 0.3);

        if (hp >= maxHp) {
            this.logMessage('HP is already full!');
            this.turn = 'player';
            return;
        }

        this.config.player.hp = Math.min(maxHp, hp + healAmount);
        this.logMessage(`Used healing herb! Restored ${healAmount} HP.`);
    ...
    private enemyAttack(): void {
        // AI pattern selection based on enemy HP ratio
        const hpRatio = this.config.enemy.hp / this.config.enemy.maxHp;
        const roll = Math.random();

        if (hpRatio < 0.3 && roll < 0.3) {
            // Desperate: strong attack when low HP
            this.enemySpecialAttack(1.5);
        } else if (roll < 0.15) {
            // Defend (reduce next incoming damage)
            this.enemyDefend();
        } else if (roll < 0.25 && this.config.enemy.maxHp > 20) {
            // Special attack (stronger enemies only)
            this.enemySpecialAttack(1.25);
        } else {
            // Normal attack
    ...
    private enemyNormalAttack(): void {
    ...
    private enemySpecialAttack(multiplier: number): void {
        this.logMessage(`Enemy uses SPECIAL ATTACK!`);

        this.animateLunge(this.enemySprite, this.eOriginalX, this.eOriginalY, -1, () => {
            const baseDmg = Math.max(1, this.config.enemy.atk - this.config.player.def);
            const dmg = Math.floor(baseDmg * multiplier);
            this.config.player.hp -= dmg;

            this.triggerShake(15);
            InputManager.vibrate(0, 400, 0.8, 1.0);

            this.spawnDamageNumber(this.pOriginalX, this.pOriginalY - 50, dmg, 0xff00ff);
            if (AudioManager.isLoaded('puzzle_tetris')) AudioManager.playSound('puzzle_tetris', { volume: 0.4 });

            this.playerSprite.tint = 0xff00ff;
            setTimeout(() => this.playerSprite.tint = 0xffffff, 300);

            this.updateHealthBars();
            this.logMessage(`Special attack deals ${dmg} damage!`);

            setTimeout(() => this.checkVictory(true), 1000);
        });
    }

    private enemyDefending = false;

    private enemyDefend(): void {
        this.logMessage(`Enemy takes a defensive stance!`);
        this.enemyDefending = true;

        this.enemySprite.tint = 0x4488ff;
