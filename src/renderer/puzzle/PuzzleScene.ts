...
export class PuzzleScene extends Scene<PuzzleSceneConfig> {
  ...
  private setupGameEvents(): void {
    ...
    this.game.on('linesCleared', (lines: number[], chain: number, combo: number) => {
      ...
      // T-spin display
      if (this.game.lastTSpin >= 1) {
          const tspinLabel = this.game.lastTSpin >= 2 ? 'T-SPIN' : 'T-SPIN MINI';
          popupText = tspinLabel + ' ' + popupText;
          popupColor = this.game.lastTSpin >= 2 ? 0xffff00 : 0xaaffaa;
          scale += 0.3;
