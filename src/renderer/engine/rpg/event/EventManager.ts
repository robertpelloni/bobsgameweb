...
export class EventManager {
    ...
    /** Callback for SHOW_MESSAGE commands — set by the scene rendering dialogue */
    private _onShowMessage: ((text: string) => void) | null = null;

    /** Set the message display callback. The DemoWorld/Scene registers this. */
    setShowMessageCallback(cb: (text: string) => void): void {
        this._onShowMessage = cb;
    }
    ...
    update(): void {
        this.activeEvents = this.activeEvents.filter(e => {
            ...
            const cmd = e.getCurrentCommand();
            if (cmd) {
                const cmdStr = cmd.commandString;
                if (cmdStr === 'SHOW_MESSAGE' && cmd.parameters.length > 0) {
                    const text = String(cmd.parameters[0]?.value ?? '');
                    if (this._onShowMessage) {
                        this._onShowMessage(text);
                    }
                    e.advance();
                } else if (cmdStr === 'SET_FLAG' && cmd.parameters.length >= 2) {
                    const flagId = cmd.parameters[0]?.asNumber() ?? -1;
                    const value = cmd.parameters[1]?.asNumber() ?? 0;
                    this.setFlag(flagId, value !== 0);
                    e.advance();
                } else {
