...
export class RealTileset {
	...
	async load(): Promise<void> {
		try {
			...
			// Debug: verify atlas pixel data for (1,1,1) transparency key
			const _dbgT = 839;
			const _dbgC = _dbgT % 256,
				_dbgR = Math.floor(_dbgT / 256);
			const _dbgX = _dbgC * 8,
				_dbgY = _dbgR * 8;
			const _dbgD = this.atlasCtx.getImageData(_dbgX, _dbgY, 8, 8);
			console.log(
				`[RealTileset] DBG Tile839 px0: RGBA(${_dbgD.data[0]},${_dbgD.data[1]},${_dbgD.data[2]},${_dbgD.data[3]})`,
			);

			this._loaded = true;
			console.log(
				`[RealTileset] Loaded atlas: ${img.width}x${img.height}, layout: native tileId positions (BUILD ${RealTileset.BUILD_VER})`,
			);
		} catch (e) {
	...
	getTileTexture(tileId: number): Texture | null {
		...
		// Debug: log first few tile textures with pixel replacement
		if (this.tileTextureCache.size < 3) {
			console.log(
				`[RealTileset] DBG getTileTexture(${tileId}): px0=RGBA(${imageData.data[0]},${imageData.data[1]},${imageData.data[2]},${imageData.data[3]}), px4=RGBA(${imageData.data[4]},${imageData.data[5]},${imageData.data[6]},${imageData.data[7]})`,
			);
		}
