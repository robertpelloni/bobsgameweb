 ...
 *
 * The Java engine renders shadow layers (L2, L5) by compositing each
 * chunk with shadowAlpha (=150/255 ≈ 0.59). Shadow tiles on L2 and L5
 * are drawn directly on their own layers with reduced alpha.
 *
 * L3 and L6 black tiles are solid opaque furniture silhouettes
 * (curtains, desk edges, bed frames, door frames) in the Java engine,
 * NOT translucent shadows. They render normally on their own layers.
 *
 * Rendering approach:
 * - L2 (groundShadow): tiles rendered with sprite.alpha = 0.59
 * - L5 (objectShadow): tiles rendered with sprite.alpha = 0.59
...
export class GameMap {
  ...
  private renderLayerReal(l: number) {
    ...
    // Shadow layers (L2, L5) render tiles with alpha=0.59 per Java engine
    const isShadowLayer =
    ...
    for (let y = startY; y < endY; y++) {
      for (let x = startX; x < endX; x++) {
        ...
        // Shadow layers (L2, L5): render translucent (Java shadowAlpha = 150/255 ≈ 0.59)
        if (isShadowLayer) {
          sprite.alpha = 0.59;
        }
  ...
  public renderViewportAround(
    ...
    for (const s of detailTiles)
      this.objectDetailContainer.removeChild(s);
    ...
    for (const l of renderableLayers) {
      ...
      // Shadow layers (L2, L5) render tiles with alpha=0.59
      const isShadowLayer =
        l === MapData.MAP_GROUND_SHADOW_LAYER ||
        l === MapData.MAP_OBJECT_SHADOW_LAYER;
      ...
      for (let y = startY; y < endY; y++) {
        for (let x = startX; x < endX; x++) {
          ...
          // Shadow layers: render translucent
